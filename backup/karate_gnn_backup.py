...
# h = h.cpu()
#
# visualize_embedding(h.cpu(), color=data.y.cpu(), epoch=0, loss=loss.cpu())
visualize_embedding(h, color=data.y, epoch = 0)
...
for epoch in range(401):
    loss, h = train(data)
    if epoch % 50 == 0:
        visualize_embedding(h.cpu(), color=data.y.cpu(), epoch=epoch, loss=loss.cpu())
        time.sleep(0.3)
